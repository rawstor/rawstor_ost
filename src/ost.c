...
prep_and_send_resp_frame(int fd, commands_t cmd, uint16_t cid, int res) {
    ...
    struct io_uring_sqe* sqe = get_sqe(&ring);
    io_uring_prep_write(
        sqe, fd, wreq->iovecs[0].iov_base, wreq->iovecs[0].iov_len, 0
    );
...
int buffer_in_stream(
    conn_t* conn, proto_io_frame_t* frame, void* buf, int len
) {
    memcpy(conn->in_buf + conn->in_bytes, buf, len);
    conn->in_bytes += len;

    LOG_DEBUG(
        "in_bytes: %i, now_len:%i, total_len:%i\n", conn->in_bytes, len,
        frame->len
    );
...
int push_block_write(conn_t* conn, proto_io_frame_t* frame, void* buf) {
    ...
    if (frame->hash != hash) {
        FLOG_INFO(
            stderr, "[%d]Write request data hash mismatch: %zu != %lu\n",
...
int process_recv(
    ...
    if (uring_unlikely(conns[fd] == NULL)) {
        LOG_DEBUG(
            "[%d]process_recv: packet from already disconnected client, "
            "ignore\n",
            fd
        );
        goto error;
    ...
    while (processed_size < res) {
        if (conn->op == NULL) {
            ...
            if (uring_unlikely(conn->op_partial_offset)) {
                new_part_size =
                    MIN(res, MAX_IN_FRAME_SIZE - conn->op_partial_offset);
                memcpy(
                    conn->op_partial_buf + conn->op_partial_offset, fdata,
                    new_part_size
                );
            ...
            if (uring_unlikely(fres < sizeof(proto_min_frame_t))) {
                FLOG_INFO(
                    stderr,
                    "[%d]Recv data is less than minimal op frame: %i < %lu, "
                    "use "
                    "op partial buffer\n",
            ...
            if (uring_unlikely(mframe->magic != RAWSTOR_MAGIC)) {
                FLOG_INFO(
                    stderr,
                    "[%d]FATAL! Frame with wrong magic number:%x != %x, "
                    "disconnect.\n",
            ...
            case CMD_SET_OBJECT: {
                if (uring_unlikely(fres < sizeof(proto_basic_frame_t))) {
                    FLOG_INFO(
                        stderr,
                        "[%d]CMD_READ recv: data len:%i differs from frame "
                        "size:%lu, use op partial buffer",
                        fd, fres, sizeof(proto_basic_frame_t)
                    );
                    goto partial_res;
                }
            ...
            default: {
                if (uring_unlikely(fres < sizeof(proto_io_frame_t))) {
                    FLOG_INFO(
                        ...
                        "[%d]CMD_READ recv: data len:%i differs from frame "
                        "size:%lu, use op partial buffer",
                        fd, fres, sizeof(proto_io_frame_t)
                    );
                    goto partial_res;
                }
                ...
                LOG_DEBUG(
                    "[%d]process_recv: set new op:%i cid:%u\n", fd,
                    conn->op->cmd, conn->op->cid
                );

                processed_size =
                    sizeof(proto_io_frame_t) - conn->op_partial_offset;
        ...
        case CMD_WRITE: {
            ...
            LOG_DEBUG(
                "CMD_WRITE: got part of data, use slow path with buffer. "
            ...
            if (conn->in_bytes > conn->op->len) {
                FLOG_INFO(
                    stderr,
                    "[%d]Unexpected state: conn->in_bytes:%u > "
                    "conn->op->len:%u\n",
        ...
        default:
            FLOG_INFO(
                stderr, "[%d]Unknown proto command: %i\n", fd, conn->op->cmd
            );
            goto error;
            break;
        }
...
void io_process_read(int fd, int res, io_request_t* ioreq) {
    ...
    LOG_DEBUG(
        "[%d]io_process_read: cid:%u res:%i len:%li hash: %lu\n", fd,
        ioreq->cid, res, ioreq->iovecs[0].iov_len + ioreq->iovecs[1].iov_len,
        hash
    );
...
static int handle_cqe(struct ctx* ctx, struct io_uring_cqe* cqe) {
    ...
    case REQ_KIND_ACCEPT: {
        ...
        if (res < 0) {
            ...
            /* hello! client address is in the req object, because that's what
             * we pointed the request to in io_uring_prep_accept */
            LOG_INFO(
                "[%d]Connect from %s:%d\n", res, inet_ntoa(areq->addr.sin_addr),
                ntohs(areq->addr.sin_port)
            );
            ...
            /* Don't try to reuse this ioreq! It'll be reused for
             * recv_multishot! */
        ...
        io_uring_prep_accept(
            sqe, server_fd, (struct sockaddr*)&areq->addr, &areq->addrlen, 0
        );
    ...
    case REQ_KIND_READ: {
        /* Important - ioreq will be reused for same connection in
         * recv_multishot, don't touch it! */
        /* get a handle on the more specialised request */
        ...
        /* zero read, they gracefully closed the connection */
            ...
            if (!(cqe->flags & IORING_CQE_F_MORE) && conns[fd] != NULL) {
                LOG_DEBUG(
                    "Incoming cqe didn't have IORING_CQE_F_MORE flag! Recreate "
                    "recv event\n"
                );
            ...
            if (ret) {
                FLOG_DEBUG(
                    stderr, "[%d]process_recv returned %i, disconnect client\n",
                    fd, ret
                );
    ...
    case REQ_KIND_WRITE: {
        ...
        if (res < 0) {
            FLOG_INFO(
                stderr, "[%d](%u)REQ_KIND_WRITE: err %s\n", fd, ioreq->cid,
                strerror(-res)
            );
            io_req_free(ioreq);
            ...
            FLOG_DEBUG(
                stderr, "[%d](%u)REQ_KIND_WRITE success: %i\n", fd, ioreq->cid,
                res
            );
    ...
    default: {
        FLOG_INFO(
            stderr,
            "[%d]handle_cqe: get cqe without useful user_data? "
            "req->event:%i, res:%i, user_data:%llu\n",
...
int main(int argc, char** argv) {
    ...
    if (setsockopt(server_fd, IPPROTO_TCP, TCP_NODELAY, &onoff, sizeof(onoff)) <
        0) {
        perror("setsockopt");
        exit(1);
    }
    ...
    */
    ...
    if (setsockopt(
            server_fd, SOL_SOCKET, SO_SNDBUF, &socketbuf_size,
            sizeof(socketbuf_size)
        )) {
        perror("setsockopt");
        exit(1);
    }
    if (setsockopt(
            server_fd, SOL_SOCKET, SO_RCVBUF, &socketbuf_size,
            sizeof(socketbuf_size)
        )) {
        perror("setsockopt");
        exit(1);
    ...
    LOG_INFO(
        "Tried to set new write socket buffer size actual size: %d\n",
        socketbuf_size
    );
     ...
     * understand the completion queue entry (CQE) that comes back, and submit
     * it for processing */
    struct io_uring_sqe* sqe = get_sqe(&ring);
    accept_request_t* req = accept_req_new(server_fd);
    io_uring_prep_accept(
        sqe, server_fd, (struct sockaddr*)&req->addr, &req->addrlen, 0
    );
    ...
    while (1) {
        ...
        /* We already processed what we could, now wait for new events, we'll
